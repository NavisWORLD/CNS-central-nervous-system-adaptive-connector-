...
class CnsMobileEngine {
  ...
  List<double> encode12(SensoryInput s) {
    final rms = _unit(s.audioRms, 0, 1);
    final freq = _unit(s.frequencyHz, 0, 4000);
    final flat = _unit(s.spectralFlatness, 0, 1);
    ...
    final phiH = math.cos(2 * math.pi * ((freq * phi) % 1)) * (0.25 + 0.75 * rms);
    final motion = _unit(s.motionEnergy, 0, 1);
    final luminance = _unit(s.luminance, 0, 1);
    final mlc = (1 - math.min(1, (motion - luminance).abs())) * 2 - 1;
    final ent = _unit(s.entropy, 0, 1) * 2 - 1;
    final frequencyMass = math.sqrt(math.max(rms, 0)) * (2 * freq - 1);
    return <double>[
      frequencyMass,
      phase,
      2 * flat - 1,
  ...
  List<double> expand42(List<double> x12, Map<String, double> context) {
    ...
    final spread = math.sqrt(x12.map((v) => math.pow(v - mean, 2).toDouble()).reduce((a, b) => a + b) / 12);
          ...
          : i % 5 == 1
              ...
              : i % 5 == 2
                  ...
                  : i % 5 == 3
                      ? energy
                      : spread;
      extra.add(math.tanh(0.62 * source + 0.23 * partner + 0.15 * control + 0.08 * harmonic));
    }
  ...
  List<double> adapt54(List<double> x42, double entropy) {
    ...
    final next = List<double>.filled(54, 0);
    for (var i = 0; i < 42; i++) {
    ...
    for (var i = 0; i < 12; i++) {
      ...
      next[42 + i] = math.tanh(0.50 * base + 0.28 * prior + 0.17 * memory + 0.05 * ent + jitter);
    }
